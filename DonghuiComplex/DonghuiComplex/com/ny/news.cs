    ...
    public class news
    {
        public static int addNews(string title, string subtitle, string photo, string content)
        {
            ...
            SqlParameter[] sp = new SqlParameter[] {
                new SqlParameter ("type","ny_news"),new SqlParameter ("title",title),
        ...
        public static DataTable getNewsList(string currentIndex, string pageCount)
        {
            string strSql = string.Format(@" select top {0} * from (
                                ...
                                where deleted=0 and type='ny_news'
        ...
        {
            string strSql = string.Format(@"update News set subtitle=@subtitle,title=@title,photo=@photo,content=@content where id=@id and type='ny_news'");
            SqlParameter[] sp = new SqlParameter[] {
        ...
        {
            string strSql = " select COUNT(*) from News where  deleted=0 and type='ny_news'";
            SQLServerOperating s = new SQLServerOperating();
        ...
        {
            string strSql = "select id, type, title, subtitle, photo, content, created, deleted from News where type = 'ny_news' and deleted = 0 and id=" + id;
            SQLServerOperating s = new SQLServerOperating();
            return s.Selects(strSql);
        }
