    ...
    public class services
    {
        public static int addServices(string title, string subtitle, string photo, string content)
        {
            ...
            SqlParameter[] sp = new SqlParameter[] {
                new SqlParameter ("type","ym_services"),new SqlParameter ("title",title),
        ...
        public static DataTable getServicesList(string currentIndex, string pageCount)
        {
            string strSql = string.Format(@" select top {0} * from (
                                ...
                                where deleted=0 and type='ym_services'
        ...
        {
            string strSql = string.Format(@"update News set subtitle=@subtitle,title=@title,photo=@photo,content=@content where id=@id and type='ym_services'");
            SqlParameter[] sp = new SqlParameter[] {
        ...
        {
            string strSql = " select COUNT(*) from News where  deleted=0 and type='ym_services'";
            SQLServerOperating s = new SQLServerOperating();
        ...
        {
            string strSql = "select id, type, title, subtitle, photo, content, created, deleted from News where type = 'ym_services' and deleted = 0 and id=" + id;
            SQLServerOperating s = new SQLServerOperating();
            return s.Selects(strSql);
        }
    }
